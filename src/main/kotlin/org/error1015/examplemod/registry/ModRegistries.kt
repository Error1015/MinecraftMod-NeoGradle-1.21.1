...
object ModRegistries {
    val SpellRegistryKey: ResourceKey<Registry<Spell>> = ResourceKey.createRegistryKey<Spell>(
        "spells".asResourceLocationPath()
    )

    val SpellRegistry: Registry<Spell> = RegistryBuilder<Spell>(SpellRegistryKey)
        .defaultKey("empty".asResourceLocationPath())
        .sync(true)
        .maxId(256)
        .create()

    val EXAMPLE_CONFIGURED_FEATURE: ResourceKey<ConfiguredFeature<*, *>?> = ResourceKey.create<ConfiguredFeature<*, *>?>(
